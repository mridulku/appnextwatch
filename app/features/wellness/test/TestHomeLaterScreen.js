...
import useSetupStatus from '../home/useSetupStatus';
...
function TestHomeLaterScreen({ navigation }) {
  ...
  const switchTab = (tabName, params) => {
    const rootTabs = navigation.getParent()?.getParent();
    if (rootTabs) {
      rootTabs.navigate(tabName, params);
      return;
    }
    const testStack = navigation.getParent();
    if (testStack && tabName === 'Sessions') {
      testStack.navigate('TestSessionsLater', params);
    }
  };
...
export default TestHomeLaterScreen;
