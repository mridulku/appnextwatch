...
function AddExercisesScreen({ navigation }) {
  const { user } = useAuth();
  ...
  const renderHeader = () => (
    <View style={styles.headerContent}>
      ...
      <Text style={styles.subtitle}>Pick from catalog exercises to personalize your gym hub.</Text>
  ...
  const renderRow = ({ item }) => {
    ...
    return (
      <CatalogCardRow
        title={item.name}
        subtitle={`${normalizeExerciseCategory(item)} • ${item.type || 'exercise'} • ${item.equipment || 'bodyweight'}`}
        selected={isAdded}
  ...
  const renderEmpty = () => {
    ...
    return <Text style={styles.emptyText}>No exercises match this filter.</Text>;
  };
